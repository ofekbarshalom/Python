...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        server_socket.bind((host, port)) # connect the socket to this host and port
        server_socket.listen(5)  # maximum number of queued clients connections
        ...
        while True:
            try:
                ...
                # return 2 values, client_socket: a new socket for communication with the connected client,
                # address: tuple of (ip address, port)
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            # receive data from the client_socket with the maximum amount of data
            data = client_socket.recv(api.BUFFER_SIZE)
            # if there is no more data, exit the loop
            if not data:
                print("No data received, exiting the loop.")
                break
