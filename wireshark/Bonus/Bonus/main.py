...
# Force Matplotlib to use a non-blocking backend (for PyCharm)
...
#################### Testing on Mixed Datasets (Bonus Task) ####################
mixed_files = [
...
for mixed_file, test_results_df in test_results_list:
    ...
    if not test_results_df.empty:
        ...
        # Shift bars left and right for actual and predicted values
        ...
        ax.set_ylabel("Count")
        ax.set_title(f"Scenario 1: Actual vs Predicted Counts ({mixed_file})")
        ax.set_xticks(x)
