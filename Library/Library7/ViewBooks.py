...
def load_books_from_file(file_path):
    try:
        ...
        print(f"File not found: {file_path}")  # Crucial: Notify if file is missing
        return None
...
def open_view_books_window():
    ...
    selected_file = tk.StringVar()  # Correctly initialize as a StringVar
    ...
    def refresh_table():
        ...
        if not selected_option:
            print("No file selected!")  # Crucial: Notify if no file is selected
            return
        file_path = file_options.get(selected_option)  # Match dropdown value to path
        if file_path:
            update_table_with_pandas(table, file_path)
        else:
            print(f"Invalid selection: {selected_option}")  # Crucial: Notify if invalid selection
