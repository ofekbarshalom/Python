...
def alert_loaned_books(book_title, loaned_books_df,books_df, alert_label):
    if book_title in loaned_books_df["title"].values:
        ...
        if client_name:  # If a name is provided
            if utils.update_book_waiting_list(book_title, client_name, books_df):
...
def lend_selected_book(tree, alert_label):
    ...
    if alert_loaned_books(book_title, loaned_books_df,books_df, alert_label):
        raise
