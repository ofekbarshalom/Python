...
class FileHandler:
    ...
    @staticmethod
    def init_popular_books(books_df, output_file=Paths.POPULAR_BOOKS.value):
        """
        Selects the top 10 books with the highest 'loaned_count' and saves them to a CSV file
        with only the 'title' and 'requests' columns.
        """
        # Ensure the required columns exist
        required_columns = {"title", "loaned_count"}
        if not required_columns.issubset(books_df.columns):
            raise ValueError(f"The DataFrame must have the columns: {', '.join(required_columns)}.")

        # Get the top 10 books by 'loaned_count'
        top_books = books_df.nlargest(10, "loaned_count")[["title", "loaned_count"]]

        # Rename 'loaned_count' to 'requests'
        top_books = top_books.rename(columns={"loaned_count": "requests"})

        # Save the top books to a CSV file
    ...
    def read_csv_files():
        ...
        try:
            ...
            # books.csv does not exist. Returning an empty DataFrame.
        ...
        try:
            ...
            # available_books.csv does not exist. Returning an empty DataFrame.
        ...
        try:
            ...
            # loaned_books.csv does not exist. Returning an empty DataFrame.
    ...
    def read_popular_books_file():
        ...
        try:
            ...
            # popular_books.csv does not exist. Returning an empty DataFrame.
