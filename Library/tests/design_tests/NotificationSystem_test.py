...
class TestNotificationSystem(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_users_df = pd.DataFrame({
            ...
            "password": [FileHandler.hash_password("password1"), FileHandler.hash_password("password2")],
    ...
    # ----------------------------------------
    @patch("design.NotificationSystem.pd.DataFrame.to_csv")  # Mock saving to CSV
    def test_notify_observers(self, mock_to_csv):
        # Add mock observers to the notification system
        self.notification_system.add_observer(self.mock_observer_1)
        self.notification_system.add_observer(self.mock_observer_2)

        # Call notify_observers with a test message
        self.notification_system.notify_observers("Test message", self.mock_users_df)

        # Assert each observer's update method was called with the correct arguments
        ...
        # Assert that the DataFrame was not saved to the real file
