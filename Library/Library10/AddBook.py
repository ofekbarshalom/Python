...
from Books_dict import add_book_to_dict
...
def submit_new_book(entries, dropdown_var, current_window):
    try:
        ...
        # Add book to book dictionary
        add_book_to_dict(new_book)

        # Close the current window
        current_window.destroy()
    except Exception:
        log.error("book added fail")
...
def open_add_new_book_window():
    ...
    submit_button = tk.Button(
        ...
        fg="white",
        width=20,
        command=lambda: submit_new_book(entries, dropdown_var, new_book_window),
    )
