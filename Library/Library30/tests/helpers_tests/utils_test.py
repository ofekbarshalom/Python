...
class TestUtils(unittest.TestCase):
    ...
    @patch("helpers.utils.utils.get_book_with_lowest_requests_and_loans", return_value=("Book X", 2))
    @patch("helpers.utils.utils.check_and_update_book_in_popular_books")
    def test_update_book_waiting_list(self, mock_check_update, mock_get_lowest, mock_to_csv, mock_calculate):
        # Mock data
        data = {
            "title": ["Book A"],
            "waiting_list": ["empty"],
            "loaned_count": [1],
            "requests": [5],  # Include 'requests' column
        }
        books_df = pd.DataFrame(data)
        file_path = "/fake/path.csv"

        # Call the function
        result = utils.update_book_waiting_list("Book A", "ofek", books_df, file_path)

        # Assertions
        self.assertTrue(result)
        self.assertEqual(books_df.at[0, "waiting_list"], "ofek", "Waiting list should be updated correctly.")
        self.assertTrue(mock_to_csv.called, "The DataFrame should be saved to the specified file path.")
        mock_check_update.assert_called_once_with("Book A", 3)  # 2 (waiting list size) + 1 (loaned count)
