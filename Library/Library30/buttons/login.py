...
from helpers.MessagesPopUp import MessagesPopUp
from start.library import open_library_window  # local import
...
def on_button_click(current_window):
    ...
    current_window.after(1000, transition)  # Delay by 1 second the close
...
def submit_login(username_entry, password_entry, current_window):
    """
        Validate login and open the library if successful.
    """
    ...
    if FileHandler.check_login(name, password):
        ...
        def proceed():
            ...
            MessagesPopUp.show_messages_pop_up(name)
            open_library_window()
...
def open_login_window():
    """
        Open the login window for the library system.
    """
    # Main window setup
    login = tk.Tk()
