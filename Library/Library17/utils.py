...
class utils:
    ...
    def show_messages_pop_up(user_name):
        ...
        pop_up = tk.Tk()
        utils.center_window(pop_up, 400, 300)
        pop_up.title(f"Messages for {user_name}")
        pop_up.configure(bg="#f2f2f2")

        # Display messages in a Label
        header = tk.Label(pop_up, text=f"Messages for {user_name}", font=("Arial", 16, "bold"), bg="#4b0082", fg="white")
        header.pack(fill=tk.X, pady=(10, 10))

        messages_label = tk.Label(pop_up, text=messages, font=("Arial", 12), bg="#f2f2f2", wraplength=350, justify="left")
        messages_label.pack(pady=10, padx=10)
