...
class FileHandler:
    ...
    def create_books_dict_from_csv(file_path=Paths.BOOKS.value):
        ...
        for _, row in df.iterrows():
            ...
            books_dict[row['title']] = book  # Use title as the key

        return books_dict

    @staticmethod
    def create_users_dict_from_csv(file_path=Paths.USERS.value):
        users = {}
        ...
        # Iterate through the DataFrame rows
        for _, row in df.iterrows():
            user_obj = User(name=row['name'], password=row['password'])
            users[row['name']] = user_obj  # Use name as the key and User object as the value
