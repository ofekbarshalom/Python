...
class FileHandler:
    ...
    def init_popular_books(books_df, output_file=Paths.POPULAR_BOOKS.value):
        ...
        """
        # Ensure the 'loaned_count' column exists
        if "loaned_count" not in books_df.columns:
            raise ValueError("The DataFrame must have a 'loaned_count' column.")

        # Get the top 10 books by 'loaned_count'
        top_books = books_df.nlargest(10, "loaned_count")

        # Save the top books to a CSV file
        top_books.to_csv(output_file, index=False)

        print(f"Top 10 popular books have been saved to {output_file}.")
