...
class TestUser(unittest.TestCase):
    ...
    def test_user_initialization(self):
        self.user = User(name="testuser", password="securepassword123")
        self.hashed_password = FileHandler.hash_password("securepassword123")
        self.assertEqual(self.user.name, "testuser", "User name is not set correctly.")
        self.assertEqual(self.user.password, self.hashed_password, "User password is not set correctly.")
        self.assertEqual(self.user.messages, [], "Messages should be initialized as an empty list.")
