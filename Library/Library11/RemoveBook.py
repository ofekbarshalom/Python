...
from Books_dict import remove_book_from_dict
...
def remove_selected_book(tree, books_df, alert_label):
    ...
    updated_books_df = books_df[books_df["title"] != book_title]
    ...
    updated_books_df.to_csv(Paths.BOOKS.value, index=False)

    # Reload the Treeview
    load_books(tree, updated_books_df)

    # Remove the book from book dictionary
    remove_book_from_dict(book_title)
