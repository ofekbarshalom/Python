...
def update_loaned_book_and_available_books(book_title):
...
def add_message_to_users(book_title):
    message = f"The book {book_title} as been removed from the library"
...
def remove_book_from_csvs(books_df, book_title):
    # Remove the book from the DataFrame
    books_df = books_df[books_df["title"] != book_title]

    update_loaned_book_and_available_books(book_title)
...
def remove_selected_book(tree, books_df, alert_label):
    ...
    if copies == copies_available:
        ...
        books_df = remove_book_from_csvs(books_df, book_title)

        add_message_to_users(book_title)
