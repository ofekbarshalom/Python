...
def load_books_from_file(file_path):
    """
        Load books data from a CSV file into a Pandas DataFrame.
    """
    try:
...
def update_table_with_pandas(table, file_path):
    """
        Update the Treeview table with data from the specified CSV file.
    """
...
def refresh_table(param, file_options, table):
    """
        Refresh the Treeview table based on the selected file option with helper methods.
    """
    if not param:
...
def open_view_books_window():
    """
        Open the "View Books" window to display book data from chosen csv.
    """
    view_books_window = tk.Tk()
    utils.center_window(view_books_window, 1400, 600)
    ...
    refresh_button = tk.Button(
        ...
        bg="#4b0082",
        fg="white",
        command=lambda: refresh_table(file_selector.get(), file_options, table)  # Call refresh_table on button click
    )
