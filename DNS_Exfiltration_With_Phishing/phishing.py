"""
Phishing Email Generator with Attachment and Template Options

This script is designed for educational cybersecurity labs.
It generates and sends a phishing-style email using one of four template options:

1. Load template from a URL
2. Load from a local .txt file
3. Enter the email text manually
4. Use an adaptive template based on user input (name, job, kids, etc.)

The final email includes:
- Customized subject and body based on placeholders like {username}, {job_title}, etc.
- An optional file attachment (e.g., .exe or .zip) simulating a malicious payload

Email is sent via a local SMTP server such as MailHog (port 1025).

How to use:
1. Ensure MailHog is running on your machine (http://localhost:8025)
2. Run the script:  python phishing_with_attachment_full_menu.py
3. Choose template source and provide required details
4. Confirm preview and choose a file to attach

DISCLAIMER: FOR LEGAL, AUTHORIZED, AND EDUCATIONAL USE ONLY.
"""
