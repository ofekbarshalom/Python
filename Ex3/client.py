...
def split_message(message, max_size):
    ...
    for i in range(0, len(message_bytes), max_size):
        chunk = message_bytes[i:i + max_size]  # Get the chunk of max_size or smaller
        result.append(chunk.decode('utf-8', errors='ignore'))  # Decode and add to the result
    return result
