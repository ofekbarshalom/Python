...
def handle_max_msg_size_request(connection_socket):
    ...
    connection_socket.send(str(max_msg_size).encode('utf-8'))  # Send the maximum size to the client
    return max_msg_size
...
def handle_file_request(connection_socket, file_path):
    ...
    connection_socket.send("ok".encode('utf-8'))  # Send to the client the server received the maximum message size
    return max_msg_size
...
def handle_packet_reception(connection_socket, max_msg_size):
    ...
    while True:
        ...
        decoded_msg = msg_from_client.decode('utf-8')
        ...
        connection_socket.send(ack_message.encode('utf-8'))
...
def server(server_address):
    ...
    while True:
        ...
        keep_sending = connection_socket.recv(1024).decode('utf-8').strip().lower()
