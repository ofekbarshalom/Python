...
def split_message(message, max_size):
    message_bytes = message.encode('utf-8')  # Encode the message in UTF-8
...
def handle_file_mode(client_socket):
    ...
    server_response = client_socket.recv(1024).decode('utf-8')  # Receive server response
...
def handle_manual_mode(client_socket):
    ...
    client_socket.send(max_ask_server.encode('utf-8'))
...
def get_acknowledge(client_socket):
    try:
        ...
        ack_from_server = client_socket.recv(1024).decode('utf-8')  # Receive acknowledgment
        ...
        if window_moved:  # Send packets within the window if the window has moved
            for i in range(window_start, min(window_start + int(window_size), len(packets))):
                ...
                if not packetsACK[i]:
                    ...
                    client_socket.send(packets[i].encode('utf-8'))  # Send the packet
